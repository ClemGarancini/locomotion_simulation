...
class BaseDisplacementSensor(sensor.BoxSpaceSensor):
    ...
    def __init__(
        ...
        """
        # TODO?
        self._channels = channels if channels else ["x", "y", "z"]
...
class FootContactSensor(sensor.BoxSpaceSensor):
    """A sensor that reads the foots contacts of a robot"""
    ...
    def _get_observation(self) -> _ARRAY:
        return np.array(self._robot.GetFootContacts()).astype(np.float32)


class MotorVelocitySensor(sensor.BoxSpaceSensor):
    """A sensor that reads motor velocity from the robot."""
    ...
    def __init__(
        ...
        self._last_motor_angle = np.zeros(self._num_motors)
        self._current_motor_angle = np.zeros(self._num_motors)

    def _get_observation(self) -> _ARRAY:
        angle_velocity = self._current_motor_angle - self._last_motor_angle
        return angle_velocity

    def on_step(self, env):
        self._last_motor_angle = self._last_motor_angle
        if self._noisy_reading:
            self._current_motor_angle = self._robot.GetMotorAngles()
        else:
            self._current_motor_angle = self._robot.GetTrueMotorAngles()

    def on_reset(self, env):
        if self._noisy_reading:
            self._current_motor_angle = self._robot.GetMotorAngles()
            self._last_motor_angle = self._robot.GetMotorAngles()
        else:
            self._current_motor_angle = self._robot.GetTrueMotorAngles()
            self._last_motor_angle = self._robot.GetTrueMotorAngles()
